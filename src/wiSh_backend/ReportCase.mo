...
actor ReportCase {
  ...
  let reports : Map.Int<CrimeReport> = Map.empty<Int, CrimeReport>();
  ...
  public func addReport(location: Text, date: Int, incidentType: Text, description: Text, victimName: ?Text, offenderPicture: ?Blob) : async Int {
    ...
    reports.put(reportId, { id = reportId; location = location; date = date; incidentType = incidentType; description = description; victimName = victimName; offenderPicture = offenderPicture; });
    return reportId;
  };
  ...
  public func sendWishCoins(amount: Int, recipient: Text) : async () {
    wishCoinsBalance -= amount;
    transactionHistory := Array.append(transactionHistory, [{ type = "send"; amount = amount; recipient = ?recipient; }]);
  };

  public func receiveWishCoins(amount: Int) : async () {
    wishCoinsBalance += amount;
    transactionHistory := Array.append(transactionHistory, [{ type = "receive"; amount = amount; recipient = null; }]);
  };
